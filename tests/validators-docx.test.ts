...
describe("DOCXSchemaValidator", () => {
    ...
    describe("validateRedundantCellBorders", () => {
        ...
        it("deduplicates multiple matching tags in nested <w:del> structures", async () => {
            await withTempDir(async (dir) => {
                await writeFile(
                    path.join(dir, "word", "document.xml"),
                    wrapDocument(
                        `<w:p><w:del w:id="1"><w:del w:id="2"><w:r><w:t>badT</w:t><w:instrText>badInstr</w:instrText></w:r></w:del></w:del></w:p>`,
                    ),
                );
                const v = new DOCXSchemaValidator({ unpackedDir: dir });
                const result = await v.validateDeletions();
                expect(result.valid).toBe(false);
                expect(result.issues.filter((i) => i.code === "del-contains-t")).toHaveLength(1);
                expect(result.issues.filter((i) => i.code === "del-contains-instrtext")).toHaveLength(1);
            });
        });
    });
