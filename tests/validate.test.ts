...
describe("validate", () => {
  ...
  it("throws when the original is not a real file", async () => {
    await withTempDir(async (dir) => {
      ...
      await expect(
        validate(docxPath, { original: path.join(dir, "missing.docx") }),
  ...
  it("flags paraId overflow in a packed DOCX (negative case)", async () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.issues.some((i) => i.severity === "error" && i.code === "id-paraid-overflow"),
    ).toBe(true);
  });
  ...
  it("word-valid profile downgrades Word-tolerated paraId overflow", async () => {
    ...
    expect(
      result.issues.some((i) => i.severity === "warning" && i.code === "id-paraid-overflow"),
    ).toBe(true);
  });
  ...
  it("word-valid profile keeps commentsIds/commentsExtensible mismatches fatal", async () => {
    ...
    expect(
      result.issues.some(
        (i) => i.severity === "error" && i.code === "comment-thread-commentid-paraid-orphan",
      ),
    ).toBe(true);
    expect(
      result.issues.some(
        (i) => i.severity === "error" && i.code === "comment-thread-durableid-orphan",
      ),
    ).toBe(true);
  });
  ...
  it("word-valid profile flags body-level m:oMathPara with m:sPre", async () => {
    ...
    expect(
      wordValid.issues.some((i) => i.severity === "error" && i.code === "word-math-spre-body"),
    ).toBe(true);
  });
  ...
  it("word-valid profile flags invalid content types that Word refuses", async () => {
    ...
    expect(
      badResult.issues.some(
        (i) => i.severity === "error" && i.code === "word-content-type-invalid",
      ),
    ).toBe(true);
    ...
    await withTempDir(async (tmp) => {
      ...
      expect(
        after.issues.some(
          (i) =>
            i.code === "repair-plan" &&
            i.path === "word/document.xml" &&
            i.message.includes("add xml:space='preserve'"),
        ),
      ).toBe(true);
...
describe("word-valid profile — applyWordValidProfile coverage", () => {
  ...
  it("flags Default element with invalid- ContentType under word-valid profile (Issue J)", async () => {
    ...
    await withTempDir(async (dir) => {
      ...
      expect(result.valid).toBe(false);
      expect(
        result.issues.some((i) => i.severity === "error" && i.code === "word-content-type-invalid"),
      ).toBe(true);
    });
  });
  ...
  it("keeps rels-broken error for ../customXml/ targets under word-valid profile (Issue K)", async () => {
    ...
    await withTempDir(async (dir) => {
      ...
      const relsIssue = result.issues.find(
        (i) => i.code === "rels-broken" && i.message.includes("../customXml/"),
      );
