...
describe("all fixtures — lenient profile", () => {
  ...
  for (const entry of manifest.entries) {
    ...
    it(`[${label}] ${entry.relativePath}${suffix}`, { timeout: 60_000 }, async () => {
      ...
      expect(result.valid, `expected ${expected.valid ? "valid" : "invalid"} in lenient mode`).toBe(
        expected.valid,
      );
  ...
  afterAll(() => {
    ...
    for (const gap of strictGaps) {
      const codes =
        gap.warningCodes.length === 0
          ? "(no warnings emitted — investigate)"
          : gap.warningCodes.join(", ");
