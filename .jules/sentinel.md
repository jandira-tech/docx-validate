...
## 2026-07-28 - Replace Math.random with crypto.randomInt for ID generation
**Vulnerability:** Insecure randomness using Math.random() for ID generation.
**Learning:** Math.random() is not cryptographically secure and can generate predictable IDs, leading to potential ID collisions.
**Prevention:** Always use node:crypto randomInt for generating secure IDs and durable tokens.
