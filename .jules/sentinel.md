...
## 2026-06-22 - Zip Slip (Path Traversal) Vulnerability in Zip Extraction

**Vulnerability:** In `src/scripts/office/validators/redlining.ts`, the code unpacked a zip file (`document.docx`) using `JSZip` by directly joining the destination directory (`tempDir`) with `entry.name`. Since zip files can contain path traversal sequences like `../../`, a malicious `.docx` file could cause the app to extract arbitrary files outside of the intended `tempDir`, potentially overwriting critical system files or planting backdoors.
**Learning:** This is a classic Zip Slip vulnerability that specifically manifests when manually unpacking archive formats like DOCX/PPTX, which are fundamentally just zip files. While they seem like benign document files, they can harbor dangerous paths.
**Prevention:** Always validate that the fully resolved destination path for an extracted entry strictly starts with the resolved target directory path (using `${path.sep}` to avoid suffix collisions, like allowing `/tmp/dest_malicious` when intending `/tmp/dest`). Throw an error immediately if the path escapes.
