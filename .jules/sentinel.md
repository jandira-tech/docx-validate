...
## 2026-07-04 - Predictable Random Number Generation for Object IDs
**Vulnerability:** The random IDs used as durable and paragraph IDs in comments and table of contents components were generated using Math.random() which is predictable and NOT cryptographically secure, leading to possible collision attacks and ID predictability.
**Learning:** Generating random IDs for potentially sensitive object anchors should always use a cryptographically secure random number generator (CSPRNG), especially in libraries that mutate files.
**Prevention:** Replaced Math.random() usage with node:crypto randomInt() to ensure secure, collision-resistant random variables.
