...
## 2026-07-17 - Insecure Randomness in Durable Token Generation

**Vulnerability:** The codebase was generating hexadecimal IDs and durable tokens (used for tracked changes and comment anchors) using `Math.random()` scaled up by bitwise operations (e.g., `Math.floor(Math.random() * 0x7ffffffe)`). `Math.random()` relies on a non-cryptographic PRNG (V8's xorshift128+) which is inherently predictable. While these IDs are not authentication secrets, generating deterministic or highly predictable document identifiers exposes the system to collision attacks or token guessing, especially when generating large volumes of IDs concurrently or in automated workflows.
**Learning:** Even for non-secret internal identifiers, relying on `Math.random()` is an anti-pattern. Predictable IDs can break data integrity logic if attackers intentionally forge matching identifiers in collaboratively edited documents.
**Prevention:** Always use cryptographically secure pseudo-random number generators (CSPRNG), such as `randomInt` or `randomBytes` from `node:crypto`, to generate unique durable identifiers and tokens to prevent predictability and collision issues.
