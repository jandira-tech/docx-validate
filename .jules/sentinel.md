...
## 2026-07-14 - Predictable Random Identifier Generation
**Vulnerability:** The use of `Math.random()` for generating identifiers like `durableId` or `paraId` produced predictable tokens. Since `Math.random()` is not cryptographically secure, an attacker could potentially guess these identifiers, leading to collisions or predictable IDs, which can cause issues with conflict resolution, concurrent editing features, or allow manipulation of these tokens.
**Learning:** Relying on standard pseudo-random number generators (PRNGs) like `Math.random()` for generating persistent identifiers or tokens introduces collision and predictability risks, particularly in environments handling multi-user document revisions.
**Prevention:** Always use cryptographically secure random number generators, such as `randomInt` from Node's `node:crypto`, when generating unique identifiers, security tokens, or any random values where predictability constitutes a risk.
