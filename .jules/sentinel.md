...
## 2026-05-19 - Path Traversal Vulnerability in Relationship Targets
**Vulnerability:** The `resolveRelationshipTargetPath` function resolved paths without verifying they stayed within the intended extraction directory. An attacker could craft a malicious document with a relationship target pointing outside the unpacked directory (e.g., `../../../etc/passwd`), leading to unauthorized file access.
**Learning:** When resolving file paths from untrusted input (like document relationships), it's not enough to just normalize or join the paths. The resolved path must be validated to ensure it does not escape the intended boundary directory.
**Prevention:** Always validate resolved paths against the intended root directory using `path.relative` and ensuring the result does not start with `..` or is absolute.
