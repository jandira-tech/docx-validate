...
## 2026-07-22 - Insecure PRNG for Document Tokens
**Vulnerability:** The application used `Math.random()` to generate IDs and durable tokens in `src/scripts/comment.ts` and `src/scripts/office/validators/docx.ts`. `Math.random()` is not cryptographically secure, which could allow an attacker to predict generated tokens and potentially manipulate or forge document parts.
**Learning:** `Math.random()` should never be used for security-sensitive operations or token generation where unpredictability is required, even for internal IDs if they are used as durable identifiers.
**Prevention:** Always use a cryptographically secure pseudo-random number generator (CSPRNG), such as `randomInt` from `node:crypto`, for generating IDs, tokens, or any values requiring unpredictability.
