...
## 2024-10-24 - Path Traversal (Zip Slip) in docx target resolution
**Vulnerability:** In `resolveRelationshipTargetPath` (`src/scripts/office/validators/docx.ts`), `path.resolve` was used without checking if the resulting target path escaped the intended `unpackedDir`.
**Learning:** `path.resolve` alone does not prevent directory traversal; it merely normalizes the path. It resolves `..` segments, meaning untrusted inputs like `Target="../../../evil.txt"` could point outside the intended root directory, which is dangerous when unpacking office files.
**Prevention:** Always validate resolved paths against the intended root directory using `path.relative` and check if the relative path starts with `..` or is absolute to block paths that escape the root.
