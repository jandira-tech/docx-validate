...
## 2026-08-01 - Predictable Token Generation Vulnerability
**Vulnerability:** The use of `Math.random()` for generating IDs (e.g., `w14:paraId`, `w16cid:durableId`) creates predictable tokens. An attacker who observes a sequence of generated IDs could potentially predict past or future IDs, leading to token collision or tracking vulnerabilities.
**Learning:** `Math.random()` is not cryptographically secure and should not be used for security-sensitive tokens, IDs, or any values requiring unpredictability.
**Prevention:** Always use the `node:crypto` module (e.g., `randomInt`) for generating IDs, tokens, or other random values to ensure they are cryptographically secure.
