...
## 2026-05-27 - Path Traversal in XML Relationship Targets

**Vulnerability:** When parsing `.rels` files, relationship targets were resolved and accessed without verifying if the resolved path stayed within the intended extraction directory. This allowed a malicious Office document with a crafted relationship (e.g., `Target="../../../etc/passwd"`) to traverse outside the directory and access arbitrary files on the filesystem.
**Learning:** XML relationship targets are user-controlled input. blind path resolution (`path.resolve`) against them without boundary checks leads to path traversal vulnerabilities.
**Prevention:** To prevent Path Traversal when resolving untrusted paths (like XML relationship targets), always validate the resolved path against the intended root directory using `path.relative` and ensure the result does not equal or start with `..` and is not absolute.
