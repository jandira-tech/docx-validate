...
## 2026-08-08 - Insecure Randomness in Identifier Generation

**Vulnerability:** The codebase was using `Math.random()` to generate hex IDs and durable IDs (`src/scripts/comment.ts`, `src/scripts/office/validators/docx.ts`). `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Predictability of identifiers can potentially lead to ID collisions or allow an attacker to guess identifiers, which could be abused depending on the context of those IDs. Secure alternatives should be used.
**Prevention:** Avoid using `Math.random()` for generating IDs or durable tokens. Use `randomInt` from `node:crypto` instead to prevent predictability and collision issues.
