...
## 2026-05-19 - Predictable Pseudo-Random Number Generator for ID Generation
**Vulnerability:** The codebase was using `Math.random()` to generate structural IDs (e.g. `paraId`, `durableId`). Since `Math.random()` is a predictable pseudo-random number generator (PRNG), it could allow an attacker to predict generated IDs or purposely cause ID collisions, which might lead to document structure manipulation or referencing issues.
**Learning:** `Math.random()` should never be used where unpredictability or non-collision is required, especially in document generation/modification scenarios where IDs are used for security references or tracking structural components.
**Prevention:** Always use cryptographically secure random number generators like `randomInt` or `randomBytes` from `node:crypto` to generate secure, unpredictable values.
