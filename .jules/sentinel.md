...
## 2026-05-20 - Predictable ID Generation using Math.random

**Vulnerability:** The codebase was generating `durableId` and `paraId` identifiers using `Math.random()`, which is a deterministic and predictable pseudo-random number generator (PRNG). If an attacker could observe a sequence of generated IDs, they could potentially predict future IDs or cause ID collisions.
**Learning:** Using `Math.random()` for generating persistent or unique identifiers is unsafe because it lacks cryptographic entropy, making the generated values predictable and susceptible to collision attacks or guessing.
**Prevention:** Always use a cryptographically secure pseudo-random number generator (CSPRNG), such as `randomInt` or `randomBytes` from `node:crypto`, when generating unique IDs, tokens, or any values that require unpredictability.
