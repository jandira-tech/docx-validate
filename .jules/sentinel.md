...
## 2026-07-24 - Predictable Identifiers via Math.random()

**Vulnerability:** The codebase was using `Math.random()` to generate durable document identifiers (e.g. `generateHexId` and in DOCX validations). `Math.random()` is not a cryptographically secure Pseudo-Random Number Generator (CSPRNG), leading to predictable values that attackers could exploit for collision attacks or forging identifiers.
**Learning:** Hardcoding or using insecure PRNGs for any form of unique and/or sensitive ID generation poses a security risk, particularly in generating document internals, which can lead to predictability and forging.
**Prevention:** Always use secure random generation libraries such as `node:crypto` (e.g., `randomInt()`) to generate IDs or tokens to ensure unpredictability.
