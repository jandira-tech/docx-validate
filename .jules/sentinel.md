...
## 2024-05-24 - Path Traversal in XML Relationship Targets

**Vulnerability:** The application resolved XML relationship targets (`target` attribute in `Relationship` tags) directly against the `unpackedDir` without validating if the resulting path escaped the output directory. An attacker could craft a malicious DOCX file with a `Target` attribute like `../../../etc/passwd` to read arbitrary files on the system during validation.
**Learning:** Even when reading ZIP entries safely (preventing Zip Slip during extraction), parsing the extracted files and blindly resolving paths found inside them (like XML relationships) against the file system re-introduces path traversal vulnerabilities.
**Prevention:** Always validate resolved paths against the intended root directory using `path.relative` to ensure the result does not equal or start with `..` and is not absolute, preventing path traversal attacks when resolving untrusted paths.
