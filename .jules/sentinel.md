...
## 2024-05-31 - Insecure Random Number Generation

**Vulnerability:** Found `Math.random()` used to generate IDs (`paraId`, `durableId`). `Math.random()` is not cryptographically secure and predictable.
**Learning:** Using `Math.random()` for IDs can lead to predictability and collision issues. It's important to use cryptographically strong pseudo-random data generators for security-sensitive or unique identifiers.
**Prevention:** Use `randomInt` from `node:crypto` instead of `Math.random()` when generating IDs or durable tokens to prevent predictability.
