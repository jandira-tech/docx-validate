...
## 2026-05-19 - Weak Randomness in ID Token Generation

**Vulnerability:** The functions used to generate and repair comment IDs (`generateHexId` in `comment.ts` and ID repairing in `docx.ts`) previously relied on `Math.random()`. `Math.random()` provides pseudorandom values and is not cryptographically secure, allowing potential predictability of generated ID tokens and collisions.
**Learning:** Using predictable randomness for durable unique identifiers can lead to token collisions and makes deterministic attacks easier if the values are leveraged in security logic downstream.
**Prevention:** Always use cryptographically secure random number generators such as `node:crypto`'s `randomInt` to construct tokens or IDs for sensitive functionality.
