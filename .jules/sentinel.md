...
## 2026-05-19 - Path Traversal in XML Relationship Targets
**Vulnerability:** XML `<Relationship>` tags could contain `Target` attributes with relative path traversal (`../`) that resolved outside the application's intended unpacked directory during validation.
**Learning:** Hardcoded `path.resolve` calls without boundary checks allow paths to escape intended boundaries, potentially leading to information disclosure or validation bypass.
**Prevention:** Always validate resolved paths against the intended root directory using `path.relative` and check for `..` or absolute paths.
