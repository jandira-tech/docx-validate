...
## 2026-05-20 - Insecure Random Number Generation for IDs

**Vulnerability:** The code used `Math.random()` to generate IDs and durable tokens (`w14:paraId`, `w16cid:durableId`). `Math.random()` is not cryptographically secure and can lead to predictability and collision issues.
**Learning:** Using predictable random numbers for generating unique identifiers can open up vectors for ID collisions or prediction attacks, especially when these IDs are used for durable tracking or document merging.
**Prevention:** Always use cryptographically secure random number generators (e.g., `randomInt` from `node:crypto`) when generating IDs or durable tokens to ensure uniqueness and unpredictability.
