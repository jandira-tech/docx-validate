...
## 2026-05-19 - Insecure Random Number Generation for Durable IDs
**Vulnerability:** The application used `Math.random()` to generate durable IDs (like `w14:paraId` and `w16cid:durableId`). Since `Math.random()` is predictably seeded and not cryptographically secure, attackers could predict generated IDs or deliberately cause ID collisions.
**Learning:** Generating tokens or unique identifiers (even if not strictly secrets) with `Math.random()` can lead to predictability and collision vulnerabilities, breaking assumptions of uniqueness across documents.
**Prevention:** Always use cryptographically secure pseudo-random number generators (CSPRNG), such as `randomInt` from `node:crypto`, when generating IDs or durable tokens to prevent predictability and collision issues.
