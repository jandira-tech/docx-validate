...
## 2026-09-15 - Path Traversal Vulnerability in XML Relationship Resolution

**Vulnerability:** The `resolveRelationshipTargetPath` function in `src/scripts/office/validators/docx.ts` resolved external document relationships without validating if the resolved path stayed within the unpacked directory bounds. An attacker could craft a malicious DOCX file containing a relationship pointing to an external directory (e.g., `Target="../../../../../etc/passwd"`). When the validator processed this file, it would attempt to access arbitrary files on the host file system.
**Learning:** This is a classic path traversal (Zip Slip variant) vulnerability where user-supplied path fragments in XML structures are blindly passed to `path.resolve` without subsequent bounds checking against the intended sandbox directory.
**Prevention:** When resolving untrusted paths (like XML relationship targets), always validate the resolved path against the intended root directory using `path.relative`. Ensure the result does not equal or start with `..` and is not absolute before allowing access. This pattern must be applied to all XML path resolution logic in the codebase.
