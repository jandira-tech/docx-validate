...
## 2026-05-19 - Path Traversal File Existence Oracle

**Vulnerability:** Relationship targets in .rels files were resolved and validated using `existsSync` without checking if they stayed within the unpacked directory boundary. This created a path traversal vulnerability acting as a file existence oracle for arbitrary files on the local filesystem.
**Learning:** Even if a file is not read or extracted, checking for its existence (or file type via `statSync().isFile()`) based on user-controlled paths outside the intended scope exposes sensitive filesystem information.
**Prevention:** Always validate resolved paths against the intended root directory using `path.relative()` and rejecting paths that start with `..` or are absolute, even for simple existence checks.
