...
## 2026-05-19 - Path Traversal in Office Relationships
**Vulnerability:** The `Target` attribute in Office `.rels` files was directly resolved without ensuring it remained within the unpacked directory, creating a Zip Slip/Path Traversal risk.
**Learning:** Relying purely on `path.resolve()` is insufficient. When dealing with untrusted user input within ZIP-like archives, paths can easily escape the destination directory by using `..` sequences.
**Prevention:** Always validate resolved paths using `path.relative()` against the intended root directory, and ensure the resulting relative path is not absolute and does not start with `..`.
