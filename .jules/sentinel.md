...
## 2026-05-27 - Predictable ID Generation using Math.random()

**Vulnerability:** The codebase was using `Math.random()` to generate IDs for comments and durable IDs. `Math.random()` is not cryptographically secure, meaning its output is predictable. This could allow an attacker to predict generated IDs and potentially cause collisions or manipulate data references.
**Learning:** For any ID or token generation that requires uniqueness and non-predictability, especially in security-sensitive contexts, relying on standard pseudo-random number generators like `Math.random()` is insufficient.
**Prevention:** Always use a Cryptographically Secure Pseudo-Random Number Generator (CSPRNG), such as `randomInt` from the `node:crypto` module in Node.js, to ensure unpredictability and prevent collision attacks.
