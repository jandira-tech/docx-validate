...
## 2026-10-27 - Insecure Randomness in ID Generation
**Vulnerability:** The application used `Math.random()` to generate unique identifiers (`paraId`, `durableId`) for DOCX comments and internal auto-repair structures.
**Learning:** `Math.random()` is a weak pseudo-random number generator that produces predictable values. Although document IDs are not high-value secrets, predictable generation can allow attackers to guess IDs and potentially manipulate document structures or cause collisions, compromising document integrity.
**Prevention:** Always use Cryptographically Secure Pseudo-Random Number Generators (CSPRNG), such as `crypto.randomInt()`, when generating any unique IDs or identifiers, even if they are not explicitly used for authentication or authorization.
