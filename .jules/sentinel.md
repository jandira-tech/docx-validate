...
## 2026-05-19 - Weak Random Number Generation for Security IDs

**Vulnerability:** Weak random number generation using `Math.random()` was used to generate durable and paragraph IDs in the application. Because `Math.random()` is not cryptographically secure, it could lead to predictability and collision issues.
**Learning:** For identifiers where predictability poses a risk or strong uniqueness is required, standard PRNGs like `Math.random()` are insufficient.
**Prevention:** Use `randomInt` from `node:crypto` or `crypto.getRandomValues()` instead to generate cryptographically secure random values and mitigate the risk.
