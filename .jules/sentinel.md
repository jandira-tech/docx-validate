...
## 2026-05-19 - Insecure Random Number Generation for Durable Tokens

**Vulnerability:** The codebase was generating random numbers for document IDs (`durableId`, `paraId`, etc.) using `Math.random()` in both `src/scripts/comment.ts` and `src/scripts/office/validators/docx.ts`. `Math.random()` is not cryptographically secure, and the V8 implementation exposes PRNG states across calls. This could allow an attacker to predict generated identifiers or token sequences by observing preceding outputs, leading to identifier collisions or potential predictability attacks on tracked-changes integrity.
**Learning:** For values that serve as permanent object references across collaborative environments, using unpredictable pseudo-random numbers is crucial for defense-in-depth against ID collisions and spoofing.
**Prevention:** Never use `Math.random()` to generate IDs or tokens. Always use `crypto.randomInt()` or `crypto.randomBytes()` for cryptographically secure random values.
