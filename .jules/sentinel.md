...
**Vulnerability:** The function `resolveRelationshipTargetPath` parsed XML relationship file targets dynamically. Since it didn't strictly validate that the resolved output file path remained within the root unpacked document directory, a malicious `.docx` file using traversal characters (`../`) could construct targets outside the intended extraction sandbox.
**Learning:** XML validation and unpacking routines must sanitize all resolved directory structures, similar to Zip Slip extraction handling. Using `path.resolve` alone is insufficient when dealing with untrusted user input within ZIPs or XML packages.
**Prevention:** Always evaluate untrusted paths by checking the outcome against a strict sandbox boundary using `path.relative` to ensure the final path remains within the bounds (i.e. does not start with `..` or become absolute).
