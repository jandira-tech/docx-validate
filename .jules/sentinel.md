...
## 2026-07-15 - Insecure Randomness in Durable IDs

**Vulnerability:** The application used `Math.random()` to generate hex IDs and durable IDs for OOXML nodes. `Math.random()` is not cryptographically secure, meaning the sequence of numbers it generates can be predicted. In contexts where IDs serve as durable markers or tracking tokens, predictability could lead to identifier collisions or allow an attacker to guess identifiers for spoofing or tracking manipulation.
**Learning:** Using `Math.random()` for anything that requires collision resistance or unpredictability (even document tokens) introduces security weaknesses.
**Prevention:** Always use cryptographically secure pseudo-random number generators (CSPRNG), such as `randomInt` from `node:crypto`, when generating identifiers, tokens, or any value where predictability or collision could be problematic.
