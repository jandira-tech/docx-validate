...
**Learning:** When trying to find elements but excluding those inside specific parent trees (like <w:p> inside <w:txbxContent>), combining getElementsByTagNameNS with a parentNode while-loop for every matched element causes O(N * depth) traversals and severe performance bottlenecks. For large documents, traversing up from every element is extremely inefficient.
**Action:** Replace getElementsByTagNameNS + upward traversal with a downward iterative Depth-First Search (DFS) that skips entire subtrees when it encounters an excluded parent tag. This turns an O(N * depth) operation into an O(N) single-pass operation.
