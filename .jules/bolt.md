...
## 2026-06-09 - Avoid xpath entirely for performance inside @xmldom
**Learning:** Using xpath selectors via `$$(".//w:del//w:t", dom)` or similar expressions on large XML documents incurs a noticeable performance hit as it constructs complex evaluators and searches the tree dynamically.
**Action:** Replace any remaining `.//` or generic path lookups (like in `validateDeletions`) with native `getElementsByTagNameNS` iterating over known namespaces. It scales significantly better when extracting multiple tag combinations.
