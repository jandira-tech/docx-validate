...
const DEFAULT_LOG = path.join(
  REPO,
  "tests/fixtures/word-regenerate-invalid/regeneration-results.jsonl",
);
...
function buildCommand(): Command {
  return new Command()
    ...
    .description(
      "Run the current DOCX repair/regenerate pipeline over the Word-invalid fixture corpus",
    )
    .option(
      "--source-root <dir>",
      "Root containing copied invalid DOCX fixtures",
      DEFAULT_SOURCE_ROOT,
    )
    .option(
      "--output-root <dir>",
      "Root where regenerated DOCX files are written",
      DEFAULT_OUTPUT_ROOT,
    )
...
async function run(argv: readonly string[]): Promise<number> {
  ...
  const filtered = opts.match
    ? sources.filter((f) => path.relative(sourceRoot, f).includes(opts.match ?? ""))
    : sources;
