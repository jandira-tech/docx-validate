...
function buildCommand(): Command {
  return new Command()
    ...
    .option(
      "--out <file>",
      "JSONL log path",
      path.resolve(HERE, "..", "tests/word-probe-results.jsonl"),
    )
    .option(
      "--profile <profile>",
      "Validator profile: lenient, strict, or word-valid",
      "word-valid",
    )
...
function parseOptions(argv: readonly string[]): CliOptions {
  ...
  if (opts.profile !== "lenient" && opts.profile !== "strict" && opts.profile !== "word-valid") {
    throw new Error(
      `Invalid --profile: ${String(opts.profile)}. Must be lenient, strict, or word-valid.`,
    );
  }
  if (opts.only !== "all" && opts.only !== "validator-valid" && opts.only !== "validator-invalid") {
    throw new Error(
      `Invalid --only: ${String(opts.only)}. Must be all, validator-valid, or validator-invalid.`,
    );
  }
  return opts;
}
  ...
  for (const line of readFileSync(outFile, "utf-8").split(/\r?\n/)) {
    ...
    try {
      ...
      if (
        ...
        (err.message.startsWith("Profile mismatch") || err.message.startsWith("Legacy JSONL"))
      )
        throw err;
...
async function run(argv: readonly string[]): Promise<number> {
  ...
  const filtered = opts.match
    ? files.filter((f) => path.relative(root, f).includes(opts.match ?? ""))
    : files;
  ...
  for (const file of filtered) {
    ...
    if (!opts.dryRun) {
      ...
      for (
        let retry = 0;
        retry < validTimeoutRetries && validator.valid && word.outcome === "timeout";
        retry += 1
      ) {
        process.stderr.write(
          `    timeout on validator-valid file; retrying (${retry + 1}/${validTimeoutRetries})\n`,
        );
        word = await probeWord(file, timeoutMs, pollMs);
      }
    }
    ...
    const wordLabel = record.word
      ? `${record.word.outcome}${record.word.clean ? "" : " (not clean)"}`
      : "dry-run";
    const alignLabel =
      record.aligned === null ? "" : record.aligned ? " aligned" : ` ${record.mismatch}`;
