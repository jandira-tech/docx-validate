...
import wasmB64 from '../wasm-b64.mjs';
// PATCHED: pass the wasm bytes to Emscripten via instantiateWasm, statically
// imported here so they're available before this top-level await runs (robust
// under bundling — no globalThis, no import-order fragility). This bypasses the
// npm glue's browser fetch/streaming path (which hangs) and uses our bytes.
const __wasmBytes = typeof atob === 'function'
    ? Uint8Array.from(atob(wasmB64), (c) => c.charCodeAt(0))
    : Uint8Array.from(Buffer.from(wasmB64, 'base64'));
const libxml2 = await moduleLoader({
    instantiateWasm(imports, success) {
        WebAssembly.instantiate(__wasmBytes, imports).then((r) => success(r.instance, r.module));
        return {};
    },
});
