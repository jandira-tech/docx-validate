 ...
 */
export async function acceptChangesResult(
  inputFile: string,
  outputFile: string,
...
async function runSofficeWithTimeout(
  args: string[],
  timeoutMs: number,
...
export async function setupLibreofficeMacro(): Promise<boolean> {
  ...
  try {
    ...
    process.stderr.write(
      `Failed to setup LibreOffice macro: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return false;
  }
}
