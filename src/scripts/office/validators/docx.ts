...
export class DOCXSchemaValidator extends BaseSchemaValidator {
    ...
    async validateDeletions(): Promise<ValidationResult> {
        ...
        for (const xmlFile of this.documentXmlFiles()) {
            ...
            // The original XPath was $(".//w:del//w:t"). The "w:" prefix maps to NS.W in XPATH_NS
            // which is "http://schemas.openxmlformats.org/wordprocessingml/2006/main".
            // So we explicitly search for that namespace, instead of all WORD_PARAGRAPH_NAMESPACES
            // to perfectly match the previous logic.
            const wNamespace = WORD_2006_NAMESPACE;
            const dels = dom.getElementsByTagNameNS(wNamespace, "del");

            for (let i = 0; i < dels.length; i++) {
                const del = dels[i];

                const ts = del.getElementsByTagNameNS(wNamespace, "t");
                for (let j = 0; j < ts.length; j++) {
                    tInDel.add(ts[j]);
                }

                const instrs = del.getElementsByTagNameNS(wNamespace, "instrText");
