        ...
        for (const xmlFile of this.documentXmlFiles()) {
            ...
            for (const ns of WORD_PARAGRAPH_NAMESPACES) {
                ...
                for (let i = 0; i < delNodes.length; i++) {
                    ...
                    if (!delNode) continue;
                    for (const childNs of WORD_PARAGRAPH_NAMESPACES) {
                        const tNodes = delNode.getElementsByTagNameNS(childNs, "t");
                        for (let j = 0; j < tNodes.length; j++) {
                            const tNode = tNodes.item(j);
                            if (tNode) tInDel.add(tNode);
                        }
                        const instrNodes = delNode.getElementsByTagNameNS(childNs, "instrText");
