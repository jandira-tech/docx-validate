    ...
    for (const xmlFile of this.documentXmlFiles()) {
      ...
      for (const ns of WORD_PARAGRAPH_NAMESPACES) {
        ...
        for (let i = 0; i < delNodes.length; i++) {
          ...
          const tNodes = (del as Element).getElementsByTagNameNS(ns, "t");
          for (let j = 0; j < tNodes.length; j++) {
            const t = tNodes.item(j);
            if (t) tInDel.add(t);
          }

          const instrNodes = (del as Element).getElementsByTagNameNS(ns, "instrText");
          for (let j = 0; j < instrNodes.length; j++) {
            const instr = instrNodes.item(j);
