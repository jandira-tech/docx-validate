...
export class DOCXSchemaValidator extends BaseSchemaValidator {
    ...
    async validateDeletions(): Promise<ValidationResult> {
        ...
        for (const xmlFile of this.documentXmlFiles()) {
            ...
            // Replace slow xpath queries with fast native DOM methods.
            ...
            for (const ns of WORD_PARAGRAPH_NAMESPACES) {
                ...
                for (let i = 0; i < delNodes.length; i++) {
                    ...
                    for (const nsChild of WORD_PARAGRAPH_NAMESPACES) {
                        const ts = delNode.getElementsByTagNameNS(nsChild, "t");
                        for (let j = 0; j < ts.length; j++) {
                            const tNode = ts.item(j);
                            if (tNode) tInDel.push(tNode);
                        }

                        const instrs = delNode.getElementsByTagNameNS(nsChild, "instrText");
                        for (let j = 0; j < instrs.length; j++) {
                            const instrNode = instrs.item(j);
