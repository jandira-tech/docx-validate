...
export class DOCXSchemaValidator extends BaseSchemaValidator {
    ...
    async validateDeletions(): Promise<ValidationResult> {
        ...
        for (const xmlFile of this.documentXmlFiles()) {
            ...
            // Replace slow xpath queries with native DOM traversal (reduces dynamic subtree search from ~23s to ~50ms on large documents)
            ...
            for (const ns of WORD_PARAGRAPH_NAMESPACES) {
                ...
                for (let i = 0; i < dels.length; i++) {
                    const ts = dels[i].getElementsByTagNameNS(ns, "t");
                    for (let j = 0; j < ts.length; j++) {
                        tInDel.add(ts[j]);
                    }
                    const instrs = dels[i].getElementsByTagNameNS(ns, "instrText");
