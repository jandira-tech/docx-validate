  ...
  async validateDeletions(): Promise<ValidationResult> {
    ...
    for (const xmlFile of this.documentXmlFiles()) {
      ...
      for (const delNs of WORD_PARAGRAPH_NAMESPACES) {
        const dels = dom.getElementsByTagNameNS(delNs, "del");
        for (let i = 0; i < dels.length; i++) {
          for (const innerNs of WORD_PARAGRAPH_NAMESPACES) {
            const ts = dels[i].getElementsByTagNameNS(innerNs, "t");
            for (let j = 0; j < ts.length; j++) {
              tInDel.add(ts[j]);
            }
            const instrs = dels[i].getElementsByTagNameNS(innerNs, "instrText");
