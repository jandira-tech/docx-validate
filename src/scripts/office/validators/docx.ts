...
export class DOCXSchemaValidator extends BaseSchemaValidator {
    ...
    async validateDeletions(): Promise<ValidationResult> {
        ...
        for (const xmlFile of this.documentXmlFiles()) {
            ...
            // ⚡ Bolt Optimization: Using native DOM APIs (`getElementsByTagNameNS`) combined with
            // a `Set` for deduplication is vastly faster than using the `xpath` package's `.//`
            // descendant queries when parsing large XML documents with `@xmldom/xmldom`.
