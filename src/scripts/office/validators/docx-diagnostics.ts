...
export async function collectDocxSemanticInventory(
  unpackedDir: string,
  profile: Profile = "lenient",
): Promise<DocxSemanticInventory> {
...
export function compareDocxSemanticInventories(
  ...
  for (const counter of beforeCounters) {
    const afterCount =
      after.counters.get(counterKey(counter.path, counter.category, counter.label, counter.unit))
        ?.count ?? 0;
    ...
    issues.push({
      ...
      message:
        `Repair lost ${counter.category} '${counter.label}': ` +
        `${counter.count} → ${afterCount} (-${lost} ${counter.unit}).`,
    });
  }
...
export function buildRepairPlanIssues(issues: readonly ValidationIssue[]): ValidationIssue[] {
  ...
  for (const issue of issues) {
    ...
    const groupAction =
      action ??
      "no automatic repair is registered; validation will report it again after repair if it remains";
  ...
  return [...groups.values()].sort(compareRepairPlanGroups).map((group) => ({
    ...
    message:
      ...
      (group.supported
        ? `Auto-repair will ${group.action}.`
        : `Auto-repair will not change them because ${group.action}.`),
  }));
}

async function collectPackageAsset(
  file: string,
  rel: string,
...
function collectRelationships(
  ...
  for (let i = 0; i < rels.length; i += 1) {
    ...
    addCounter(
      ...
      rel,
      "relationship",
      `type '${shortRelationshipType(type)}'`,
      "relationship(s)",
      1,
    );
...
function collectContentTypes(
  ...
  for (let i = 0; i < overrides.length; i += 1) {
    ...
    addCounter(
      ...
      rel,
      "content type",
      `override '${elem.getAttribute("ContentType") ?? "unknown"}'`,
      "declaration(s)",
      1,
    );
  }
  ...
  for (let i = 0; i < defaults.length; i += 1) {
    ...
    addCounter(
      ...
      rel,
      "content type",
      `default '${elem.getAttribute("ContentType") ?? "unknown"}'`,
      "declaration(s)",
      1,
    );
  }
...
function collectDocumentStructure(
  ...
  const mathElements =
    dom.getElementsByTagNameNS(MATH_NAMESPACE, "oMath").length +
    dom.getElementsByTagNameNS(MATH_NAMESPACE, "oMathPara").length;
  if (mathElements > 0)
    addCounter(inventory, rel, "document structure", "math object", "element(s)", mathElements);
  const drawingElements =
    dom.getElementsByTagNameNS(WP_NAMESPACE, "inline").length +
    dom.getElementsByTagNameNS(WP_NAMESPACE, "anchor").length;
  ...
  if (pictures > 0)
    addCounter(inventory, rel, "document structure", "DrawingML picture", "element(s)", pictures);
}
...
function collectText(rel: string, dom: Document, inventory: MutableDocxSemanticInventory): void {
  ...
  for (let i = 0; i < all.length; i += 1) {
    ...
    if (local === "fldSimple") {
      const instr =
        elem.getAttributeNS(wordNamespace(elem), "instr") ??
        elem.getAttribute("w:instr") ??
        elem.getAttribute("instr");
      if (instr)
        addCounter(inventory, rel, "text", "field instruction", "character(s)", instr.length);
      continue;
    }
...
function collectFormatting(
  ...
  for (const ns of WORD_NAMESPACES) {
    ...
    for (let i = 0; i < runs.length; i += 1) {
      ...
      if (enabledChild(rPr, "strike"))
        addCounter(inventory, rel, "formatting", "strikethrough", "formatted character(s)", chars);
      if (enabledChild(rPr, "dstrike"))
        addCounter(
          ...
          "formatted character(s)",
          chars,
        );
      if (enabledChild(rPr, "caps"))
        addCounter(inventory, rel, "formatting", "caps", "formatted character(s)", chars);
      if (enabledChild(rPr, "smallCaps"))
        addCounter(inventory, rel, "formatting", "small caps", "formatted character(s)", chars);
      if (enabledChild(rPr, "vanish"))
        addCounter(inventory, rel, "formatting", "hidden", "formatted character(s)", chars);
    ...
    for (let i = 0; i < paragraphs.length; i += 1) {
      ...
      if (numPr) {
        ...
        if (numId)
          addCounter(inventory, rel, "numbering", `numbering id '${numId}'`, "paragraph(s)", 1);
        const ilvl = wordChildAttr(numPr, "ilvl", "val");
        if (ilvl)
          addCounter(inventory, rel, "numbering", `numbering level '${ilvl}'`, "paragraph(s)", 1);
      }
    }
    ...
    for (let i = 0; i < tables.length; i += 1) {
      ...
      if (tblStyle)
        addCounter(inventory, rel, "style reference", `table style '${tblStyle}'`, "table(s)", 1);
    }
  }
}
...
function collectInlineMarks(
  ...
  for (const ns of WORD_NAMESPACES) {
    ...
    for (let i = 0; i < brs.length; i += 1) {
      ...
      if (!br) continue;
      const type =
        br.getAttributeNS(ns, "type") ??
        br.getAttribute("w:type") ??
        br.getAttribute("type") ??
        "textWrapping";
      if (type === "separator" || type === "continuationSeparator") continue;
      const label =
        type === "page" ? "page break" : type === "column" ? "column break" : "line break";
    ...
    for (let i = 0; i < tabEls.length; i += 1) {
      ...
      if (
        parent &&
        parent.nodeType === 1 &&
        isWordElement(parent as Element) &&
        localName(parent as Element) === "r"
      )
        tabs += 1;
    }
...
function collectSectionGeometry(
  rel: string,
  dom: Document,
  inventory: MutableDocxSemanticInventory,
): void {
    ...
    for (let i = 0; i < sects.length; i += 1) {
      ...
      if (pgMar) {
        ...
        addCounter(
          ...
          rel,
          "section geometry",
          `section margins T${t} R${r} B${b} L${l}`,
          "section(s)",
          1,
        );
      }
...
function wordChildAttrSelf(elem: Element, attr: string): string | null {
  return (
    elem.getAttributeNS(wordNamespace(elem), attr) ??
    elem.getAttribute(`w:${attr}`) ??
...
function collectStyles(rel: string, dom: Document, inventory: MutableDocxSemanticInventory): void {
  ...
  for (const ns of WORD_NAMESPACES) {
    ...
    for (let i = 0; i < styles.length; i += 1) {
      ...
      if (!style) continue;
      const styleId =
        style.getAttributeNS(ns, "styleId") ??
        style.getAttribute("w:styleId") ??
        style.getAttribute("styleId") ??
        "unknown";
      const type =
        style.getAttributeNS(ns, "type") ??
        style.getAttribute("w:type") ??
...
function collectComments(
  ...
  for (const ns of WORD_NAMESPACES) {
    ...
    for (let i = 0; i < comments.length; i += 1) {
      ...
      const author =
        comment.getAttributeNS(ns, "author") ??
  ...
  if (commentEx > 0)
    addCounter(inventory, rel, "comment thread", "commentsExtended entry", "entry(s)", commentEx);
  const commentIds = dom.getElementsByTagNameNS(W16CID_NAMESPACE, "commentId").length;
  if (commentIds > 0)
    addCounter(inventory, rel, "comment thread", "commentsIds entry", "entry(s)", commentIds);
  const commentExtensible = dom.getElementsByTagNameNS(
    W16CEX_NAMESPACE,
    "commentExtensible",
  ).length;
  if (commentExtensible > 0)
    addCounter(
      inventory,
      rel,
      "comment thread",
      "commentsExtensible entry",
      "entry(s)",
      commentExtensible,
    );
}

function collectTrackedChanges(
  rel: string,
  dom: Document,
  inventory: MutableDocxSemanticInventory,
): void {
  ...
  for (const ns of WORD_NAMESPACES) {
    for (const local of tracked) {
      ...
      if (elems.length > 0)
        addCounter(inventory, rel, "tracked change", local, "element(s)", elems.length);
      for (let i = 0; i < elems.length; i += 1) {
        const elem = elems.item(i);
        if (!elem) continue;
        const author =
          elem.getAttributeNS(ns, "author") ??
          elem.getAttribute("w:author") ??
          elem.getAttribute("author");
        if (author)
          addCounter(inventory, rel, "tracked change", `${local} by '${author}'`, "element(s)", 1);
      }
    }
  }
}
...
function textLabel(elem: Element, local: string): string {
  ...
  if (local === "instrText")
    return hasWordAncestor(elem, "del") ? "deleted field instruction" : "field instruction";
...
function enabledChild(parent: Element, local: string): boolean {
  ...
  const val =
    child.getAttributeNS(wordNamespace(child), "val") ??
...
function wordChildAttr(parent: Element, local: string, attr: string): string | null {
  ...
  return (
    child.getAttributeNS(wordNamespace(child), attr) ??
    child.getAttribute(`w:${attr}`) ??
...
function xsdRepairAction(issue: ValidationIssue): string | null {
  ...
  if (
    message.includes("}graphic': This element is not expected") ||
    message.includes("}blipFill': This element is not expected")
  ) {
    return "insert missing inline-picture scaffolding required by WordprocessingML/DrawingML schemas";
  }
  if (
    message.includes("Expected is ( {http://schemas.openxmlformats.org/drawingml/2006/picture}spPr")
  ) {
