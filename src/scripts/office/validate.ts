...
import { BaseSchemaValidator } from "./validators/base.ts";
...
export async function validate(
	...
	if (original) {
		...
		if (!SUPPORTED_SUFFIXES.has(ext)) {
			throw new Error(
				`Error: ${original} must be a .docx, .pptx, or .xlsx file`,
			);
		}
	...
	if (!SUPPORTED_SUFFIXES.has(dispatchSuffix)) {
		throw new Error(
			`Error: Cannot determine file type from ${target}. Use --original or provide a .docx/.pptx/.xlsx file.`,
		);
	}

	const targetStat = await fs.stat(target);
	const targetSuffix = path.extname(target).toLowerCase();
	const targetIsPackedFile =
		targetStat.isFile() && SUPPORTED_SUFFIXES.has(targetSuffix);

	if (!targetIsPackedFile && !targetStat.isDirectory()) {
		throw new Error(`Error: ${target} is not a directory or Office file`);
	}

	const runWithUnpacked = async (
		unpackedDir: string,
	): Promise<ValidateRunResult> => {
...
async function extractAll(zip: JSZip, outputPath: string): Promise<void> {
	...
	for (const { name, file } of entries) {
		...
		if (
			!resolved.startsWith(`${outputPath}${path.sep}`) &&
			resolved !== outputPath
		) {
...
export function buildCommand(): Command {
	...
	cmd
		...
		.argument(
			"<path>",
			"Path to unpacked directory or packed Office file (.docx/.pptx/.xlsx)",
		)
		.option(
			"--original <file>",
...
export async function runFromArgv(argv: readonly string[]): Promise<number> {
	...
	if (opts.profile !== "lenient" && opts.profile !== "strict") {
		...
		process.stderr.write(
			`Invalid --profile: ${bad}. Must be 'lenient' or 'strict'.\n`,
		);
		return 1;
	}
	...
	if (result.valid) {
		...
		for (const issue of result.issues) {
			...
			process.stderr.write(
				`${issue.severity.toUpperCase()}${where}: ${issue.message}\n`,
			);
		}
	}
