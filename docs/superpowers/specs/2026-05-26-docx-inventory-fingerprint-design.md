...
- Wrap = `inline` when `wp:extent`'s **parent** is `wp:inline`, `anchor` when its
  parent is `wp:anchor`. `wp:extent` is always a *direct child* of those two
...
"reshape" event — a shape change (`table 3×4 → 3×2`) decomposes into a *decrease*
of `table 3×4` plus an *increase* of `table 3×2`. There are therefore only two
directions to tier.
...
| **Section geometry** *(strict only)* | page size, orientation, margins, columns | 🟠 warn | 🟠 warn |
| **Image shape** *(strict only)* | extent (bucketed), wrap | 🟠 warn | 🟠 warn |
...
loss):** a table that *grows* `3×2 → 3×4` and a table that is *deleted then
re-added at a new shape* are indistinguishable in the identity-free histogram —
...
class: the `table` / `table row` / `table cell` *counts* decrease unambiguously
...
- `formatInventoryDiffMarkdown(diff): string` — top-level sections **Added /
  ...
  display-only pairing that, within a single `(path, category)` of a *shape*
...
3. **Severity policy** (same file): `severityFor(class, direction)` —
   ...
   info. **Critical regression test:** a table *growth* `3×2 → 3×4` yields only
