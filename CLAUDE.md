...
For the avoidance of doubt, the Elements enumerated above are not the sole Non-Parseable Elements; and each time an Element is classified as a Non-Parseable Element, such classification must (i) emit a warning during code execution (at runtime), and (ii) be documented in `README.md` and `CLAUDE.md`.
