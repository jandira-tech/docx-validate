...
- The vendored `docx/` folder is a third-party library snapshot (the `docx` npm package). Its `CLAUDE.md` is unrelated to this project — don't treat it as authoritative.
...
For the avoidance of doubt, the Elements enumerated above are not the sole Non-Parseable Elements; and every time an Element is classified as a Non-Parseable Element, such classification must (i) emit a warning during code execution (at runtime), and (ii) be documented in `AGENTS.md`, `README.md` and `CLAUDE.md`.
